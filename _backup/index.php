			...
			case 'advancedSearch':
				...
				for($i=0;$i<count($temp);$i++)
				{
					...
					if(strpos($kljuc,",")===FALSE){
						...
						echo '<li><span>'.(count($temp)-$i).'</span>&nbsp;<a href="index.php?source=frontPage&action=advancedSearch&subaction=search&'.$href.'">Kljuc pretrage:'.$temp[$i]->getKljuc().'&nbsp;||&nbsp;Upit pretrage:'.$temp[$i]->getSearchString().'</a></li>';
					}
				}
				echo'</ul>';
