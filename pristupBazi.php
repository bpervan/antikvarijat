<?PHP
	...
	class pristupBazi
	{
		public static function nadjiKnjiguAdvanced($title,$author,$publisher,$language,$type)
		{
			...
			if($author!=""){
				$q=$q.'and author like "%'.$author.'%" ';
			}
			if($publisher!=""){
				$q=$q.'and publisher like "%'.$publisher.'%" ';
			}
			if($language!=""){
				$q=$q.'and language like "%'.$language.'%" ';
			}
			if($type!=""){
				$q=$q.'and type like "%'.$type.'%" ';
			}
			if($q=='select * from knjiga where '){
				return -1;
			}		
		...
		{
			$temp=array();
			$q='SELECT * FROM knjiga where '.$kljucPretrage.' like "%'.$kriterijPretrage.'%"';
			$rs=mysql_query($q);
			if($rs===FALSE){
				die($q);
				return 0;
		...
		public static function dohvatiZadnjeUpise()
		{
			$temp=array();
			$i=0;
			$q='select knjiga.* from knjiga join primjerakknjige on primjerakknjige.sifraKnjige=knjiga.sifraKnjige join unos on primjerakknjige.sifraUnosa=unos.sifraUnosa order by unos.datumUnosa desc,unos.vrijemeUnosa desc LIMIT 25';
			$rs=mysql_query($q);
			while($row=mysql_fetch_array($rs))
			{
				$knjiga=new knjiga(
		...
		public static function izbrisiKnjigu($sifraKnjige)
		{
			...
			$q='delete from primjerakknjige where sifraKnjige='.$sifraKnjige;
			$rs=mysql_query($q);
			if($rs===FALSE){
					echo $q.'<br />';			
					die(mysql_error());
